...
export async function writeFeed(alerts) {
  ...
  try {
    const r = await fetch((process.env.SITE_URL || 'https://app.usereef.io') + '/data/alerts.json?t=' + Date.now());
...
export async function sendTelegram(publicAlerts, watchAlerts, siteUrl = process.env.SITE_URL || 'https://app.usereef.io') {
